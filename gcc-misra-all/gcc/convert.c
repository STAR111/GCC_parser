...
convert_to_pointer (tree type, tree expr)
{
  ...
  switch (TREE_CODE (TREE_TYPE (expr)))
    ...
    case REFERENCE_TYPE:
      ...
      /*
       * BUAA SEI MISRA-C 2004 Rule 11.1
       * Conversions shall not be performed between a pointer
       * to a function and any type other than an integral type.
       *
       * Warn if the expression is a pointer to a function.
       * GCC only allows type of pointer and integer to which a pointer is converting.
       * So I just insert this judging into this 'convert_to_pointer' function.
       * (Integer type is permitted by this rule)
       *
       * 2011-01-06
       * @author yangmushan
       */
      if(rule[11][1] &&
          ...
          warning(1,
              "MISRA-C Rule 11.1: Conversions shall not be performed between a pointer to a function and any type other than an integral type."
              );
      }
      /* BUAA SEI MISRA-C 2004 Rule 11.1 END */

      /*
       * BUAA SEI MISRA-C 2004 Rule 11.2
       ...
       *
       * Warn if the conversion is between a pointer to object (a pointer not to void or function)
       * and a pointer to function.
       * Other situation is already prevented by GCC.
       *
       * 2011-01-06
       * @author yangmushan
       */
      if(rule[11][2] && (
             ((TREE_CODE( TREE_TYPE ( type)) != VOID_TYPE && TREE_CODE( TREE_TYPE ( type)) != FUNCTION_TYPE ) &&
             (TREE_CODE( TREE_TYPE ( TREE_TYPE ( expr))) == FUNCTION_TYPE )) ||
             ((TREE_CODE( TREE_TYPE ( TREE_TYPE ( expr))) != VOID_TYPE  && TREE_CODE( TREE_TYPE ( TREE_TYPE ( expr))) != FUNCTION_TYPE ) &&
             (TREE_CODE( TREE_TYPE ( type)) == FUNCTION_TYPE ))  )){
          warning(1,
              "MISRA-C Rule 11.2: Conversions shall not be performed between a pointer to object and any type other than an integral type, another pointer to object type or a pointer to void."
              );
      }
      /* BUAA SEI MISRA-C 2004 Rule 11.2 END */
