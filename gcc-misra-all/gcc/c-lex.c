...
/* Returns the narrowest MISRA unsigned type.  */

static enum integer_type_kind
narrowest_unsigned_underlying_type (unsigned HOST_WIDE_INT low,
			 unsigned HOST_WIDE_INT high,
			 unsigned int flags)
{
  int itk = itk_unsigned_char;
...
/* Ditto, but narrowest signed type.  */
static enum integer_type_kind
narrowest_signed_underlying_type (unsigned HOST_WIDE_INT low,
		       unsigned HOST_WIDE_INT high, unsigned int flags)
{
  int itk = itk_signed_char;
...
{
  tree value, type, underlying_type;
  enum integer_type_kind itk, iutk;
  cpp_num integer;
  ...
  if (flags & CPP_N_UNSIGNED)
    {
      ...
      iutk = narrowest_unsigned_underlying_type (integer.low, integer.high, flags);
    }
  else
    {
	...
	= narrowest_signed_type (integer.low, integer.high, flags);
      ...
      iutk = narrowest_signed_underlying_type (integer.low, integer.high, flags);
  ...
  if (iutk == itk_none)
    underlying_type = ((flags & CPP_N_UNSIGNED)
	    ? widest_unsigned_literal_type_node
	    : widest_integer_literal_type_node);
  else
    underlying_type = integer_types[iutk];
