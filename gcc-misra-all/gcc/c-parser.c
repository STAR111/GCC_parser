...
static struct c_expr c_parser_expr_no_commas (c_parser *, struct c_expr *, tree);
static struct c_expr c_parser_conditional_expression (c_parser *,
						      struct c_expr *, tree);
static struct c_expr c_parser_binary_expression (c_parser *, struct c_expr *, tree);
static struct c_expr c_parser_cast_expression (c_parser *, struct c_expr *, tree);
static struct c_expr c_parser_unary_expression (c_parser *, tree);
static struct c_expr c_parser_sizeof_expression (c_parser *);
static struct c_expr c_parser_alignof_expression (c_parser *);
static struct c_expr c_parser_postfix_expression (c_parser *, tree);
	  ...
	  if (c_parser_next_token_is (parser, CPP_EQ))
	    {
	      ...
	      enum_value = c_parser_expr_no_commas (parser, NULL, NULL).value;
	    }
	  else
  ...
  while (true)
	  ...
	  || c_parser_next_token_is_keyword (parser, RID_ATTRIBUTE))
	{
	  ...
	  if (c_parser_next_token_is (parser, CPP_COLON))
	    {
	      c_parser_consume_token (parser);
	      width = c_parser_expr_no_commas (parser, NULL, NULL).value;
	    }
	  if (c_parser_next_token_is_keyword (parser, RID_ATTRIBUTE))
	    postfix_attrs = c_parser_attributes (parser);
				  ...
				  struct c_declarator *inner)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_SQUARE))
	...
	{
	  star_seen = false;
	  dimen = c_parser_expr_no_commas (parser, NULL, NULL).value;
	}
      else
	{
...
c_parser_initializer (c_parser *parser)
{
  if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
    ...
    {
      struct c_expr ret;
      ret = c_parser_expr_no_commas (parser, NULL, NULL);
	     ...
	     || c_parser_next_token_is (parser, CPP_DOT))
	{
	  ...
	  if (c_parser_next_token_is (parser, CPP_DOT))
	      ...
	      if (des_prev == 0 && c_dialect_objc ())
		{
		  ...
		  first = c_parser_expr_no_commas (parser, NULL, NULL).value;
		  if (c_parser_next_token_is (parser, CPP_ELLIPSIS)
		  ...
		  while (c_parser_next_token_is (parser, CPP_COMMA))
		    {
		      struct c_expr next;
		      c_parser_consume_token (parser);
		      next = c_parser_expr_no_commas (parser, NULL, NULL);
	      ...
	      if (c_parser_next_token_is (parser, CPP_ELLIPSIS))
		{
		  ...
		  c_parser_consume_token (parser);
		  second = c_parser_expr_no_commas (parser, NULL, NULL).value;
		}
	      else
		second = NULL_TREE;
	      if (c_parser_next_token_is (parser, CPP_CLOSE_SQUARE))
...
c_parser_initval (c_parser *parser, struct c_expr *after)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_BRACE) && !after)
    ...
    {
      init = c_parser_expr_no_commas (parser, after, NULL);
  ...
  if (c_parser_next_token_is_keyword (parser, RID_CASE))
    {
      tree exp1, exp2;
      c_parser_consume_token (parser);
      exp1 = c_parser_expr_no_commas (parser, NULL, NULL).value;
      if (c_parser_next_token_is (parser, CPP_COLON))
	{
	  c_parser_consume_token (parser);
	...
	{
	  c_parser_consume_token (parser);
	  exp2 = c_parser_expr_no_commas (parser, NULL, NULL).value;
...
static struct c_expr
c_parser_expr_no_commas (c_parser *parser, struct c_expr *after, tree lhs_type)
{
  struct c_expr lhs, rhs, ret;
  enum tree_code code;
  location_t op_location;
  gcc_assert (!after || c_dialect_objc ());
  lhs = c_parser_conditional_expression (parser, after, lhs_type);
  ...
  c_parser_consume_token (parser);
  rhs = c_parser_expr_no_commas (parser, NULL, TREE_TYPE (lhs.value));
...
static struct c_expr
c_parser_conditional_expression (c_parser *parser, struct c_expr *after, tree lhs_type)
{
  ...
  cond = c_parser_binary_expression (parser, after, lhs_type);
  ...
  exp2 = c_parser_conditional_expression (parser, NULL, NULL);
...
static struct c_expr
c_parser_binary_expression (c_parser *parser, struct c_expr *after, tree lhs_type)
{
...
#define POP								      \
  ...
  gcc_assert (!after || c_dialect_objc ());
  stack[0].expr = c_parser_cast_expression (parser, after, NULL);
  ...
  while (true)
	...
	default:
      ...
      sp++;
      stack[sp].expr = c_parser_cast_expression (parser, NULL, NULL);
      stack[sp].prec = oprec;
      stack[sp].op = ocode;
    }
 out:
...
/* Parse a cast expression (C90 6.3.4, C99 6.5.4).  If AFTER is not
   NULL then it is an Objective-C message expression which is the
   primary-expression starting the expression as an initializer.
   *prev_cast_type means the nearest previous cast type, this arg is add
   for misra 10.5. It is NULL if there is no previous cast.
...
static struct c_expr
c_parser_cast_expression (c_parser *parser, struct c_expr *after, tree prev_cast_type)
{
  gcc_assert (!after || c_dialect_objc ());
  if (after)
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_PAREN)
	...
	return c_parser_postfix_expression_after_paren_type (parser,
							     type_name);
      expr = c_parser_cast_expression (parser, NULL, type_name->specs->type);
      expr = default_function_array_conversion (expr);
      ret.value = c_cast_expr (type_name, expr.value);
      ret.original_code = ERROR_MARK; 
      ret.parenthesis_num = expr.parenthesis_num;
      return ret;
    }
  else
    {
	  ...
	  && TYPE_PRECISION (ut) < TYPE_PRECISION (integer_type_node))
	{
	  if (prev_cast_type == NULL || prev_cast_type != ut)
	    {
...
static struct c_expr
c_parser_unary_expression (c_parser *parser, tree prev_cast_type)
{
  int ext;
  ...
  switch (c_parser_peek_token (parser)->type)
    ...
    case CPP_PLUS_PLUS:
      c_parser_consume_token (parser);
      op = c_parser_cast_expression (parser, NULL, NULL);
    ...
    case CPP_AND:
      ...
      return parser_build_unary_op (ADDR_EXPR,
				    c_parser_cast_expression (parser, NULL, NULL),
				    loc);
    case CPP_MULT:
      c_parser_consume_token (parser);
      op = c_parser_cast_expression (parser, NULL, NULL);
	...
	case RID_EXTENSION:
	  c_parser_consume_token (parser);
	  ext = disable_extension_diagnostics ();
	  ret = c_parser_cast_expression (parser, NULL, NULL);
	  restore_extension_diagnostics (ext);
	  return ret;
	case RID_REALPART:
	  c_parser_consume_token (parser);
	...
	default:
	  return c_parser_postfix_expression (parser, NULL);
	}
    default:
      return c_parser_postfix_expression (parser, prev_cast_type);
    }
}
...
c_parser_sizeof_expression (c_parser *parser)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_PAREN)
    ...
    {
      expr_loc = c_parser_peek_token (parser)->location;
      expr = c_parser_unary_expression (parser, NULL);
    sizeof_expr:
      skip_evaluation--;
      in_sizeof--;
      if (TREE_CODE (expr.value) == COMPONENT_REF
...
static struct c_expr
c_parser_postfix_expression (c_parser *parser, tree prev_cast_type)
{
  ...
  switch (c_parser_peek_token (parser)->type)
    ...
    case CPP_NUMBER:
      ...
      expr.original_code = ERROR_MARK; expr.parenthesis_num = 0;
      loc = c_parser_peek_token (parser)->location;
      c_parser_consume_token (parser);
    ...
    case CPP_WSTRING:
      expr.value = c_parser_peek_token (parser)->value;
      expr.original_code = STRING_CST; expr.parenthesis_num = 0;
      c_parser_consume_token (parser);
      break;
	...
	case RID_VA_ARG:
	  ...
	  e1 = c_parser_expr_no_commas (parser, NULL, NULL);
	...
	case RID_CHOOSE_EXPR:
	  ...
	  e2 = c_parser_expr_no_commas (parser, NULL, NULL);
	  if (!c_parser_require (parser, CPP_COMMA, "expected %<,%>"))
	    {
	      c_parser_skip_until_found (parser, CPP_CLOSE_PAREN, NULL);
	      expr.value = error_mark_node;
	  ...
	  e3 = c_parser_expr_no_commas (parser, NULL, NULL);
	  c_parser_skip_until_found (parser, CPP_CLOSE_PAREN,
				     "expected %<)%>");
	  {
	    tree c;
...
{
  struct c_expr expr;
  expr = c_parser_expr_no_commas (parser, NULL, NULL);
  while (c_parser_next_token_is (parser, CPP_COMMA))
    {
      struct c_expr next;
      c_parser_consume_token (parser);
      ...
      expr.original_code = COMPOUND_EXPR; expr.parenthesis_num = 0;
    }
  return expr;
}
...
c_parser_omp_clause_collapse (c_parser *parser, tree list)
{
  ...
  if (c_parser_require (parser, CPP_OPEN_PAREN, "expected %<(%>"))
    {
      num = c_parser_expr_no_commas (parser, NULL, NULL).value;
      c_parser_skip_until_found (parser, CPP_CLOSE_PAREN, "expected %<)%>");
    }
  ...
  if (c_parser_next_token_is (parser, CPP_COMMA))
    {
      ...
      t = c_parser_expr_no_commas (parser, NULL, NULL).value;
...
c_parser_omp_atomic (c_parser *parser)
{
  ...
  lhs = c_parser_unary_expression (parser, NULL).value;
...
c_parser_omp_for_loop (c_parser *parser, tree clauses, tree *par_clauses)
{
  ...
  for (i = 0; i < collapse; i++)
	       ...
	       && c_parser_peek_2nd_token (parser)->type == CPP_EQ)
	{
	  ...
	  decl = c_parser_postfix_expression (parser, NULL).value;

	  c_parser_require (parser, CPP_EQ, "expected %<=%>");
	  init_loc = c_parser_peek_token (parser)->location;

	  init_exp = c_parser_expr_no_commas (parser, NULL, NULL);
