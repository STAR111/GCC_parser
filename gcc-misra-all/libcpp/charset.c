 ...
 /*check rule[4][1],only convert\n,\t,\b,\r,\f,\\,\',\",\?,\a,\v*/
  switch (c)
    ...
    case 'u': case 'U':
      {
       if(rule[4][1])
         cpp_error (pfile, CPP_DL_WARNING,
		   "violate MISRA-C-2004 rule4.1,non-ISO-standard escape sequence, '\\%c'",(int)c);
    ...
    case 'x':
     {
      if(rule[4][1])
        cpp_error (pfile, CPP_DL_WARNING,
		   "violate MISRA-C-2004 rule4.1,'\\x' is non-ISO-C escape character!");
    ...
    case '4':  case '5':  case '6':  case '7':
    {
      if(rule[4][1])
        cpp_error (pfile, CPP_DL_WARNING,
		   "violate MISRA-C-2004 rule4.1,non-ISO-standard escape sequence");
