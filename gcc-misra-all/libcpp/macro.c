...
/*rule19.4 by lxl*/
static void check_in_macro_body(cpp_reader *,const cpp_macro *);
static void handle_cpp_name(cpp_reader *, const cpp_macro *);
int rule[22][18];
...
_cpp_create_definition (cpp_reader *pfile, cpp_hashnode *node)
{
 ...
 /*check rule 19.4,rule[19][4] isn't 0,not system header,not built-in header,macro body is not  null,macro token isn't zero*/
 if(rule[19][4] && ok && (macro->syshdr == 0) && (strcmp(pfile,"<built-in>")!= 0) && (&(macro ->exp)) && (macro->exp.tokens >0))
{
 check_in_macro_body(pfile,macro);
}
  return ok;
}

static void check_in_macro_body(cpp_reader *pfile,const cpp_macro *macro)
{
/*the number of '('*/
...
/*the number of '{'*/
 ...
 cpp_token *token = &macro->exp.tokens[macro->count - 1];
 cpp_token *ttoken = &macro->exp.tokens[0];
 unsigned int i;
 ...
 if(num1)
   cpp_error_with_line(pfile,CPP_DL_WARNING,macro->line,0,"violate MISRA-C rule19.4:()is not matched");
 if(num2)
   cpp_error_with_line(pfile,CPP_DL_WARNING,macro->line,0,"violate MISRA-C rule19.4:[]is not matched");
 if(num3)
   cpp_error_with_line(pfile,CPP_DL_WARNING,macro->line,0,"violate MISRA-C rule19.4:{}is not matched");  
...
/*check the last token is ';'*/
 if(token->type == CPP_SEMICOLON)
  cpp_error_with_line(pfile,CPP_DL_WARNING,macro->line,0,"violate MISRA-C rule19.4:the final token ; is unexpected!");

/*check the macro body style,include 5 kinds.if cpp_name and token is one,the token must be limited*/
 switch(ttoken->type)
 {
  case CPP_NAME:
    if(macro ->count == 1)
    handle_cpp_name(pfile,macro);
    break;
  default:
    break; 
 }
...
/*cpp_name :extern,static,auto,register,typedef,const,volatile,do*/
static void handle_cpp_name(cpp_reader *pfile,const cpp_macro *macro)
{
  cpp_token *token = &macro->exp.tokens[0];
  char *ch = token->val.node->ident.str;
  if(strcmp(ch,"extern") && strcmp(ch,"static") && strcmp(ch,"auto") &&    strcmp(ch,"register") && strcmp(ch,"typedef") && strcmp(ch,"const") && strcmp(ch,"volatile"))
{
 cpp_error_with_line(pfile,CPP_DL_WARNING,macro->line,0,"violate MISRA-C rule19.4:the macro body is unexpected cpp_name style!"); 
}
}
